...
export const formatFixedPointNumber = (value: string, digits: number): string => {
...
export const formatFloatingPointNumber = (value: string, maxDigits: number): string => {
  const parsed = parseNumber(value);
  const [head, tail] = parsed.split(".");
  // Avoid rounding errors at toLocaleString as when user enters 1.239 and maxDigits=2 we
  // must not to convert it to 1.24, it must stay 1.23
...
export const negNumberFormat = (str: string) => {
  ...
  return result ? result.toLocaleString("en") : "";
};
...
export const dateFormat = (str: string) => {
  ...
  return chars.reduce(
    (result, value, index) =>
      (index === 2 || index === 4 ? `${result}-${value}` : `${result}${value}`).substr(0, 10),
    "",
  );
};
...
export const dateFormatSym = (str: string) => {
  ...
  const result = chars.reduce(
    (value, char, index) =>
      (index === 2 || index === 4 ? `${value}-${char}` : `${value}${char}`).substr(0, 10),
    "",
  );

  if (result.length === 0) return result;

  return `${result}${dateMask.substr(result.length)}`;
};
...
export const formatPhone = (str: string) => {
  ...
  const result = usPhone.input(clean);
  usPhone.reset();
